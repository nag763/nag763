...
pub fn post_scholarship() -> impl IntoView {
    let i18n = use_i18n();
    view! {
        <div class="grid grid-rows-6 2xl:flex 2xl:flex-col items-center 2xl:items-stretch 2xl:justify-between py-4 overflow-scroll 2xl:overflow-visible">
        <h1 class="row-span-1 max-h-full text-xl 2xl:text-4xl">{t!(i18n, post_scholarship_title)}</h1>
        <ol class="relative border-s border-black dark:border-gray-800 text-left row-span-4 h-full 2xl:h-auto overflow-scroll 2xl:overflow-visible  2xl:text-left">
        <li class="mb-10 ms-4">
